...
![AI蛮子多模态提示词工作台](AIManzi-workflow.png)

## 使用

- 工作台顶部是 **文字要求**；模型与 NInfer 参数在它下方。输出仅为正向提示词纯文本。
- **推理方式** 可选择“本地推理”或“在线推理”。本地推理保持现有 NInfer/GGUF 行为；在线推理会隐藏全部本地模型选项，只显示 API URL、API Key 和模型 ID，并通过 OpenAI 兼容的 `chat/completions` 接口请求服务。
- 在线推理不会加载本地模型或占用本地推理显存；文字、模板/Skill、图像和视频输入仍然有效。API URL 可填写 `https://服务地址/v1`，也可直接填写完整的 `.../chat/completions` 地址。API Key 在节点中以密码形式显示且不会写入日志；工作流文件可能保存控件值，因此分享工作流前应清除密钥。
